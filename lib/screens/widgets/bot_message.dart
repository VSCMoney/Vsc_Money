...
class _BotMessageWidgetState extends State<BotMessageWidget>
  ...
  // Keep top footprint stable while streaming
  static const double _kHeaderReserve = 30.0; // total reserved header height
  static const double _kStatusHeight  = 18.0; // shimmer/line height inside header
  ...
  List<TextSpan> _buildFormattedSpans(String text, TextStyle base) {
    ...
    for (final m in bold.allMatches(text)) {
      ...
      spans.add(TextSpan(
        text: m.group(1),
        style: base.copyWith(fontWeight: FontWeight.w600, height: 1.5, fontFamily: "DM Sans"),
      ));
      last = m.end;
    }
    if (last < text.length) {
        ...
        if (widget.onAskVitty != null)
          TextButton(
            ...
            child: Row(
              mainAxisSize: MainAxisSize.min,
              children: const [
                Text('Ask Vitty', style: TextStyle(color: Colors.black)),
                SizedBox(width: 8),
                Icon(Icons.chat_bubble_outline, size: 18, color: Colors.black),
              ],
            ),
          ),
        TextButton(
  ...
  // Constant-height header to prevent upward shift when status disappears
  Widget _headerWithStableHeight({
    ...
    return AnimatedSize(
      ...
      child: showStatus && statusText != null && statusText.isNotEmpty
          ? Padding(
        padding: const EdgeInsets.only(bottom: 8.0),
        child: SizedBox(
          ...
          : const SizedBox.shrink(), // Completely collapse when no status
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Split message around placeholder
    ...
    // Show status only while streaming latest
    final bool showStatus = (widget.isLatest && !widget.isComplete && validStatus);
    final bool hasAnyContent = hasPreText || hasPostText || hasTable;
    ...
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // 1) Constant header block (prevents jump when status hides)
          _headerWithStableHeight(
            showStatus: showStatus,
            statusText: widget.currentStatus,
          ),

          // 2) Content (no dynamic top padding)
          if (hasPreText)
            SelectableText.rich(
              TextSpan(style: style, children: _buildFormattedSpans(preForView, style)),
              textHeightBehavior: _thb,
